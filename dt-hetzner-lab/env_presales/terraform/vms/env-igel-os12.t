...
data "vsphere_virtual_machine" "igel_os12" {
  name          = var.vsphere.image_igel_os12  # Name of the Igel OS image
  datacenter_id = data.vsphere_datacenter.dc.id    # Reference to the ID of the associated vSphere datacenter
}

# Resource block to create virtual machines of type "igel_os12"
resource "vsphere_virtual_machine" "igel_os12" {

  for_each = var.machine_igel_os12  # Create one VM for each entry in the machine_igel_os12 map
  ...
  num_cpus = 2
  memory   = 4096
  guest_id = data.vsphere_virtual_machine.igel_os12.guest_id  # Reference to the ID of the Igel OS image
  ...
  disk {
    label = "disk0"
    size  = data.vsphere_virtual_machine.igel_os12.disks.0.size  # Size of the VM disk
  }
  ...
  clone {
    template_uuid = data.vsphere_virtual_machine.igel_os12.id  # Reference to the ID of the Igel OS image
