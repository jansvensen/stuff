# Data source to get information about the Windows Server 2022 image
data "vsphere_virtual_machine" "s2022" {
  name          = var.vsphere.image_s2022      # Name of the Windows Server 2022 image
  datacenter_id = data.vsphere_datacenter.dc.id  # Reference to the ID of the associated vSphere datacenter
}

# Resource block to create virtual machines of type "s2022"
resource "vsphere_virtual_machine" "s2022" {

  for_each = var.machine_s2022  # Create one VM for each entry in the machine_s2022 map
  ...
  guest_id = data.vsphere_virtual_machine.s2022.guest_id  # Reference to the ID of the Windows Server 2022 image
  ...
  disk {
    label = "disk0"
    size  = data.vsphere_virtual_machine.s2022.disks.0.size  # Size of the VM disk
  }
  ...
  clone {
    template_uuid = data.vsphere_virtual_machine.s2022.id  # Reference to the ID of the Windows Server 2022 image
    linked_clone  = false  # Create a full clone of the template

    customize {
      windows_options {
        computer_name = "${each.value.name}"  # Set the computer name for Windows Server 2022
      }
      network_interface {
